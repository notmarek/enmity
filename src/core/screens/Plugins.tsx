                        ...
                        <View style={styles.actions}>
                            {Settings && (
                                ...
                                >
                                    <Image
                                        ...
                                        source={Assets.getIDByName("settings")}
                                    />
                                </TouchableOpacity>
                            )}
                            {plugin.sourceUrl &&
                            <TouchableOpacity
                                ...
                                onPress={(): void => {
                                    ...
                                    Plugins.installPlugin(`${plugin.sourceUrl}?${Math.random() * 101}.js`, () => {
                                        Toasts.show({
                            ...
                            <TouchableOpacity
                                style={styles.delete}
                                onPress={(): void =>
                                    void Plugins.uninstallPlugin(plugin.name)
                                }
                            >
                                <Image
                                    style={styles.trashIcon}
                                    source={Assets.getIDByName(
                                        "ic_trash_filled_16px"
                                    )}
                                />
                            </TouchableOpacity>
                            <FormSwitch
                        ...
                        Plugins.installPlugin(url, ({ data }) => {
                            ...
                            switch (data) {
                                case "fucky_wucky":
                                    ...
                                    res.icon =
                                        Assets.getIDByName("ic_close_16px");
                                    break;
...
export function Page() {
    ...
    const [plugins, setPlugins] = React.useState(
        Plugins.getPlugins().sort((a, b) => a.name.localeCompare(b.name))
    );
    ...
    const entities = search
        ? plugins.filter((p) => {
              ...
              if (
                  p.authors?.find?.((a) =>
                      (a.name ?? a).toLowerCase().includes(search.toLowerCase())
                  )
              ) {
                  return true;
              }
